...
//код gpt, который настроил реализацию перехода на главную страницу, если у человека нет курсов 
// также он попытался подключить файербейс к коду, чтобы оттуда тянуть данные 
...
export const Account = () => {
  ...
  // Функция для получения курсов из базы данных
  const fetchCourses = async () => {
    try {
      const response = await fetch("https://fitness-cee19-default-rtdb.europe-west1.firebasedatabase.app/courses.json");
  ...
  // Вызов функции получения курсов при загрузке компонента
  useEffect(() => {
    fetchCourses();
  }, []);
        ...
        <div className="w-full min-h-screen ">
          ...
          <div>
            ...
            <div className="mt-8 mb-10">
              ...
              {courses.length === 0 ? (
                <Link to={AppRoutes.MAIN} className="text-blue-500 underline">
                  Здесь пока нет курсов. Перейдите на главную страницу, чтобы подобрать занятие
                </Link>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {courses.map((course, index) => (
                    <div key={index} className="flex flex-col gap-7 bg-white rounded-3xl relative cursor-pointer shadow-md">
                      <img className="w-full" src={course.image} alt={course.name} />
                      <img className="absolute top-4 right-4" src="/img/remove.png" alt="Удалить" />
                      <div className="mx-7">
                        <h3 className="text-3xl font-semibold pb-6 text-black">{course.name}</h3>
                        <div className="flex flex-row flex-wrap gap-1 mb-2">
                          <div className="flex flex-row flex-wrap gap-2 w-28 h-9 bg-light-gray rounded-3xl items-center pl-2.5">
                            <img className="h-4 w-4" src="./img/calendar.png" alt="Календарь" />
                            <p className="text-black">{course.duration} дней</p>
                          </div>
                          <div className="flex flex-row gap-2 w-44 h-9 bg-light-gray rounded-3xl items-center pl-2.5">
                            <img className="h-4 w-4" src="./img/time.png" alt="Время" />
                            <p className="text-black">{course.time} мин/день</p>
                          </div>
                        </div>
                        <div className="flex flex-row gap-2 w-32 h-9 bg-light-gray rounded-3xl items-center pl-2.5 mb-2">
                          <img className="h-4 w-4" src="/img/difficulty.png" alt="Сложность" />
                          <p className="text-black">Сложность</p>
                        </div>
                        <div className="p-4 mt-4">
                          <p className="text-gray-600">Прогресс {course.progress}%</p>
                          <div className="w-[300px] bg-light-gray rounded-full h-2.5 mt-1">
                            <div className="bg-blue h-2.5 rounded-full" style={{ width: `${course.progress}%` }}></div>
                          </div>
                        </div>
                        <button className="bg-green mb-3 text-black w-full py-3 rounded-[46px] mt-4 hover:bg-lime-500 transition">
                          {course.progress > 0 ? "Продолжить" : "Начать тренировки"}
                        </button>
                      </div>
                    </div>
                  ))}
